import { COMPETITORS, type Config, type CompetitorConfig } from "../config.js";
...
const log = createLogger("compare-pages");

/** What counts as a mention of PostHog on someone else's page. */
const POSTHOG_ALIASES = ["posthog", "post hog"];
...
/** Per competitor, so one long comparison page cannot fill the prompt. */
const MAX_CLAIMS_PER_COMPETITOR = 6;

export function mentionsPostHog(text: string): boolean {
  const lower = text.toLowerCase();
  return POSTHOG_ALIASES.some((alias) => lower.includes(alias));
}

/**
 * The paragraphs on a competitor page that talk about PostHog, with the
 * heading they sit under. A comparison page repeats itself across a feature
 * matrix, so identical paragraphs are kept once.
 */
export function extractPostHogClaims(
  url: string,
  ...
  for (const block of blocks) {
    ...
    if (!mentionsPostHog(block.paragraph)) continue;
...
async function fetchPageClaims(
  ...
  return extractPostHogClaims(normalizeUrl(url), competitor.id, extractPage(html).blocks);
}

/**
 * Every configured comparison page for one competitor, read for what it says
 * about PostHog. A page that fails costs its claims, never the run: an alert
...
/**
 * The comparison pages for a run, fetched once per competitor. Two competitors
    ...
    async claimsFor(competitor: CompetitorId): Promise<CompetitorClaim[]> {
      ...
      if (!pending) {
        pending = fetchCompareClaims(config, COMPETITORS[competitor]).then((claims) => {
          log.info(
            `${COMPETITORS[competitor].label}: ${claims.length} claims about PostHog from ${COMPETITORS[competitor].comparePages.length} comparison pages`,
          );
          return claims;
        });
...
/** Used when the comparison pages must not be fetched, e.g. a fast local run. */
export const EMPTY_COMPARE_INDEX: CompareIndex = {
  async claimsFor(): Promise<CompetitorClaim[]> {
    return [];
  },
};
