...
Analyzer: ${analyzer}. Target channel: #posthog-competitor-happenings (\`C0C07A1DM09\`).
...
\`\`\`json
${JSON.stringify({ channel: "C0C07A1DM09", ...message, unfurl_links: false, unfurl_media: false }, null, 2)}
\`\`\`
`;
...
async function main(): Promise<void> {
  ...
  if (values.url) {
    ...
    if (values.out) {
      await writeMessageFile(values.out, values.url, message, analyzerLabel);
