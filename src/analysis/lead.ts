...
/**
 * Slack shows the first sentence of an action's detail and nothing else, so
 * that sentence carries the whole recommendation. It has to lead with the
 * work: what to change, or which page to edit and what it should say. A
 * sentence that only says what PostHog lacks leaves the reader to work out
...
/** The distinctive words in a page's own URL, e.g. "amplitude", "alternatives". */
...
function productLead(action: RecommendedAction): string {
  if (action.type === "consider_building") return "Build this into PostHog";
  const feature = action.feature ?? productsForAction(action)[0]?.label;
  return `Close this gap in ${feature ?? "PostHog"}`;
}
...
export function enforceActionLead(analysis: Analysis): LeadRewrite {
  ...
  const actions = analysis.actions.map((action): RecommendedAction => {
    ...
    if (isPageAction(action)) {
      if (namesCitedPage(lead, analysis.posthogRefs)) return action;
      const ref = pageToName(analysis.posthogRefs);
      if (!ref) return action;

      notes.push(`led a ${action.type} action with ${ref.url}, which its opening sentence left out`);
