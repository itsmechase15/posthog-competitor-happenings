...
import { contextForSignal, topUpDocsForActions } from "../posthog/docs.js";
...
class AnalystAnalyzer implements Analyzer {
  ...
  async analyze(input: AnalyzerInput): Promise<AnalyzerOutput> {
    const prompt = buildAnalysisPrompt(input.item, {
      ...
      outline: input.workspace?.outline,
    });
 ...
 * it, recommends nothing, and every message it produces says so.
 */
 ...
 * 2. One analyst run, with the whole corpus searchable on disk and a map of
 *    what is in it. What it opens is recorded.
...
export async function analyzeItems(
  ...
  for (const item of items) {
    ...
    try {
      ...
      const grounded = topUpDocsForActions(context.index, item, reply.analysis.actions, docs);
      const analysis = checkAnalysis(reply, grounded, item, context, analyzer.model, (note) =>
  ...
  if (usedUrls.size > 0) {
    await store
      ...
      .catch((error: unknown) => {
        log.warn(
          "could not stamp the pages this run used",
          error instanceof Error ? error.message : error,
        );
      });
  }
...
export function checkActions(
  ...
  // on an action that is about to be dropped. The heuristic is exempt from the
  // topic guard because it recommends nothing for the guard to judge.
  ...
  // The gate can retype nothing and drop plenty, so the sentence Slack shows
  // is shaped after it rather than before.
...
export function checkAnalysis(
  ...
  const checked = checkActions(reply.analysis, docs, item, coverage, model);
 ...
 *
 * A rewrite is a claim about what PostHog ships, exactly like the claim it
...
export function checkAction(input: ActionCheck): {
  ...
  const candidate: Analysis = {
    ...
    posthogRefs: input.refs,
  };
  const checked = checkActions(candidate, input.docs, input.item, input.coverage, input.model);
