...
// is stated once, where the message is built. The same for the bullets under
// "More detail", including the room the last one gets.
import { MAX_ACTION_CHARS, MAX_POINT_CHARS, MAX_SO_WHAT_CHARS } from "../slack/message.js";
...
- "key_points" is 2 to 4 lines of substance that go under a "More detail" heading, below the summary and the impact. Every line but the last is one fact about the piece, as a fragment under ${MAX_POINT_CHARS} characters: what it does, who it is for, what it claims, what it replaces, what it concedes. No line repeats the summary.
- The last key point is the so-what, and it is the one line allowed to run long: up to ${MAX_SO_WHAT_CHARS} characters, a sentence or two rather than a fragment. It says what the competitor is selling in this piece ${EN_DASH} the product, platform, or paid layer the argument leads to, and what a reader has to accept to buy it ${EN_DASH} written so a PostHog marketer can see what a piece positioned against it would have to answer. It matters most on a post that ships nothing, and most of all next to a consider_publishing action, where it is the brief the draft was written from.
  Good: "The argument lands on their SDK as the collection layer their analytics platform needs on top of the warehouse: session replay, heatmaps, and pre-login attribution exist only if their library is on the page, so a warehouse-first team installs it anyway. A PostHog answer has to say which of those PostHog gets from warehouse sources and which of them need posthog-js."
  Bad: "Closes by pointing at their install docs and their agent." Also bad: "A pitch for their analytics platform." Both name the pitch and leave a marketer with nothing to answer.
  Prefer one rich last line to four thin ones. Three key points whose last line is worth reading beat four whose last line is a closer.
...
- For consider_publishing, "detail" is the brief a marketer acts on, and it has two parts in this order, separated by line breaks written as \\n inside the JSON string. The piece itself goes in "article_draft" and never in "detail", which is neither long enough to hold it nor where anything looks for it.
  1. The lead: one to three short sentences. The first names the piece to write and the angle, and stands alone, because Slack shows that sentence and nothing else. The ones after it say how PostHog should position the piece against what the competitor is selling ${EN_DASH} the pitch you named in the last key point ${EN_DASH} and which PostHog products the piece rests on, by the names the docs use. A marketer reading three sentences should know what to write, what it argues against, and what it stands on.
  2. Then "**What's in the draft**" on a line of its own, and under it one markdown bullet per beat of the draft: three to five lines, each starting with "- ", each one line long, in the draft's own order. Beats, not the draft. A paragraph retelling the whole piece is what this replaces, and the piece is already in "article_draft".
  Worked example, line breaks and all: "Publish a PostHog take on whether you need an SDK or can send events from your warehouse.\\nPosition it against the pitch that the SDK is the layer a warehouse cannot replace: PostHog ships both sides, so the honest answer names what the warehouse covers and what it does not.\\nLean on warehouse sources and the capture API for what SQL-first teams already have, and on session replay, surveys, and experiments for what needs posthog-js on the page.\\n**What's in the draft**\\n- What warehouse sources and the capture API already answer.\\n- The three things that need the SDK on the page, and why.\\n- A person join resolves at query time, so flags and experiments never see it.\\n- The hybrid setup most teams land on, and when warehouse-only is right."
