...
/**
 * Some changelogs are one page with an `#anchor` per release: Mixpanel's whole
 * changelog lives at docs.mixpanel.com/changelogs, and every entry in the feed
 * points at a hash on it. `item.url` has that hash stripped, because dedupe and
 * the PostHog index want one URL per page, so the item alone can no longer say
 * which release it was about. Sources that know the anchor keep it here.
 */
