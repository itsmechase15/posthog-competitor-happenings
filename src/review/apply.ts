 ...
 *
 * It runs after the issues are opened and before the Slack post. That order is
 * the design: the issue exists, so a verdict has something to write to and the
 * whole outcome is in the issue's own history, and Slack has not gone out yet,
 * so an action the reviewer drops is simply not in the message. Slack is never
...
function agreedComment(outcome: ReviewOutcome): string {
  return [
    ...
    "",
    "The recommendation above is unchanged. A second model read PostHog's docs corpus and found nothing that contradicts it.",
  ].join("\n");
}
...
function droppedComment(outcome: ReviewOutcome, verdict: NoAction): string {
  return [
    ...
    "",
    "Closed as not planned, and left out of the Slack alert. If the pages above are out of date, that is a docs fix rather than a reason to reopen this.",
  ].join("\n");
}
 ...
 * why it closed each issue, and asserting some other reason on its behalf – for
 * years this said "because PostHog already covers this" whatever the reviewer
 * had found – is exactly the platitude the verdict exists to stop.
 */
