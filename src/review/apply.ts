...
function droppedComment(outcome: ReviewOutcome): string {
  return [
    ...
    "",
    pagesLine(outcome, "Pages that show PostHog already covers this:"),
    "",
...
function noActionReasonFor(dropped: string[], model: string | undefined): string {
  ...
  return `Every recommended action here was dropped on review${by}, because PostHog already covers this. ${dropped.join(" ")}`;
}
