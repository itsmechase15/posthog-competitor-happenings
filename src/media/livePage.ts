 ...
 * the copy to paste. What it cannot do in text is show what the page looks
 * like with the edit in it, which is the first question anybody asked to make
 * the edit has. So the real page is opened in a headless browser,
 * photographed, the proposed copy is put into that browser's own copy of the
 ...
 * beyond the GET that loads the page, and the copy to paste is still the
 * instruction the issue carries. The caption on the after shot says so,
 * because a reader seeing their own page with words they have not written on
 * it deserves to be told which of the two is real.
 *
 ...
 * challenge page, a line that is not there any more: each costs the picture
 * and none of them costs the issue, which says the same thing in words.
 */
...
/**
 * Desktop, because the compare pages are read on one and the sidebar is half
 * of what makes the shot look like posthog.com rather than a card. Twice the
 ...
 * Once, and no further: past this the paragraph is small in a tall page and
 * the PNG is a megabyte of whitespace.
 */
...
/**
 * How long the document has to stop changing before anything is marked on it.
 *
 * posthog.com is server-rendered and then hydrated, and hydration throws away
 * the nodes the server sent about a second after `domcontentloaded`. Anything
 * written onto the page before that – the attribute marking the paragraph to
 * edit, most of all – goes with them, and the after shot comes back empty
 * having been taken of a page that no longer had the mark on it. Waiting for
 * the DOM to go quiet is also what makes the pair comparable: a before shot of
 * the server's markup next to an after shot of React's is two pictures of two
 * pages.
 */
const QUIET_MS = 600;

/** And how long to wait for that quiet before taking the page as it is. */
 ...
 * where a product screenshot goes is not what the page looks like.
 */
...
export interface StageInput {
  ...
  /** The copy on the page today, verbatim, as the evidence gate checked it. */
  oldLine: string;
  proposedText: string;
  mode: "replace" | "insert";
  /**
...
export type StageOutcome =
  ...
  /** The folded line is nowhere on the page. */
  | { status: "missing" }
  /** The line is on the page but spread across two blocks, so there is no one paragraph to edit. */
  | { status: "split" }
 ...
 *
 * Located by its text and never by a CSS selector. posthog.com is redesigned,
 ...
 *
 * Self-contained to the last line, constants included: what reaches the
 * browser is this function's own source, so anything it reads from the module
 * around it is a `ReferenceError` in the page. {@link stageExpression} is how
 * it gets there.
 */
function stagePageEdit(input: StageInput): StageOutcome {
  ...
  /**
   * The block the line is on: the deepest one that holds all of it, first in
  ...
  function locate(): HTMLElement | "missing" | "split" {
    ...
    const wanted = fold(input.oldLine).folded;
  ...
  /** The quoted line as a Range over the element's own text nodes. */
  function rangeFor(element: HTMLElement): Range | null {
    ...
    const found = span(text, input.oldLine);
   ...
   * makes this a picture of posthog.com.
   */
 ...
 * lives in the module and not in the page, so the browser throws `__name is
 * not defined` – on a morning run, never under the test runner, whose
 * transform does not add it. A no-op `__name` in the closure the call is made
 * from costs a line and makes both run the same way.
 *
...
/**
 * Wait until the document stops changing, or until waiting stops being worth
 * it.
 *
 * A source string rather than a function for the reason {@link stageExpression}
 * is: what Playwright ships into the page is source, and the nested function a
 * `MutationObserver` needs would arrive wrapped in a `__name` the page has
 * never heard of.
 */
function settleExpression(quietMs: number, limitMs: number): string {
  return `new Promise((resolve) => {
    let timer = setTimeout(done, ${quietMs});
    const capped = setTimeout(done, ${limitMs});
...
async function settle(page: Page): Promise<void> {
  ...
  await page.evaluate(settleExpression(QUIET_MS, SETTLE_TIMEOUT_MS)).catch(() => undefined);
}

/** The three things the browser side needs off a plan. */
function edit(plan: PageEditPlan): Pick<StageInput, "oldLine" | "proposedText" | "mode"> {
  return { oldLine: plan.oldLine, proposedText: plan.proposedText, mode: plan.mode };
}

/**
 * Photograph one page edit: the page as it reads today, then the same page
 * with the proposed copy in the browser's copy of the document.
 *
...
export async function captureEdit(
  ...
  const context = await browser.newContext({
    ...
    // does whoever opens the issue. A dark shot of a light page is a picture
    // of somebody else's browser.
    colorScheme: "light",
  ...
  try {
    ...
    // fallback face, an animation caught mid-way is a shot that differs every
    // run, and a mark written on the page before it hydrates is a mark thrown
    // away with the markup it was written on.
    await settle(page);
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      // The new copy runs off the bottom of the window, so the after shot
      // would cut it off. Grow the window once and take the pair again, both
      // at the new size: a before and an after of two different windows is
