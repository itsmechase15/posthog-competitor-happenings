 ...
 * any PostHog page talk about this?", and the thing being matched is a
 * competitor's launch vocabulary against PostHog's docs vocabulary – the same
...
/** A product's overview page beats a guide that happens to use the same words. */
...
const STOP_WORDS = new Set(
  [
    ...
    "other", "our", "out", "over", "posthog", "see", "set", "so", "some", "than", "that", "the",
...
/**
 * The docs section a page sits in, read off its path. posthog.com puts the
 * whole corpus under one host, so `/docs/experiments/holdouts` has to name
 * `docs/experiments` rather than `docs` – otherwise one section holds three
 * thousand pages and the per-section cap stops meaning anything.
 */
export function sectionOf(url: string): string {
  try {
    ...
    if (segments[0] === "docs") return segments.slice(0, 2).join("/");
...
function countTerms(doc: Pick<PostHogPage, "title" | "text">): Map<string, number> {
 ...
 * for every item that run reads the same index: a corpus of a few thousand
 * pages costs a second or two here and nothing afterwards.
 */
export function buildCorpusIndex(pages: PostHogPage[]): CorpusIndex {
    ...
    search(query, options = {}) {
      ...
      for (const doc of docs) {
        ...
        for (const token of queryTerms) {
          ...
          matched.push(token);
          const denominator = frequency + K1 * (1 - B + (B * doc.length) / (averageLength || 1));
