...
import { fetchPostHogPage, NOT_MODIFIED } from "./fetch.js";

const log = createLogger("posthog-corpus");
 ...
 * a fortnight-old copy answers "does PostHog do this at all" just as well.
...
export function isDue(page: PageMeta, config: Config, now: Date): boolean {
  ...
  const maxAgeDays = usedRecently ? config.docsHotRefreshDays : config.posthogRefreshDays;
...
export function planRefresh(
  ...
  due.sort(
    (a, b) => (meta.get(a)?.fetchedAt.getTime() ?? 0) - (meta.get(b)?.fetchedAt.getTime() ?? 0),
  );

  const wanted = fresh.length + due.length;
  const budget = config.posthogMaxPages;
...
export interface CorpusRefreshResult {
  ...
  changed: number;
  /** Of those, how many the server answered with a 304 and no body at all. */
  unchangedByEtag: number;
  retired: number;
  gone: number;
  discovered: number;
  claims: number;
...
async function readPage(
  ...
  try {
    const body = await fetchPostHogPage(config, url, {
      etag: previous?.etag ?? undefined,
      lastModified: previous?.lastModified ?? undefined,
    });

    if (body === NOT_MODIFIED) {
      return { ...NOTHING_READ, notModified: true };
    }
 ...
 *
 * PostHog publishes a few thousand pages, and reading them one request at a
 * time is the difference between a run that takes a minute and one that takes
 * ten. The width is small on purpose: this is somebody else's website, and the
 * point is to stop being needlessly slow, not to crawl as hard as we can.
 */
...
/**
 * Refresh the corpus: discover what PostHog publishes, read what is new or
...
export async function refreshDocsCorpus(
  ...
  if (config.skipPosthogIndex) {
    log.info("skipping the corpus refresh (SKIP_POSTHOG_INDEX)");
    result.pages = await store.loadCorpus();
    result.notes.push(
      `corpus: refresh skipped, reasoning against ${result.pages.length} stored pages`,
    );
    return result;
  }
  ...
  const readThisRun = new Map<string, PostHogPage>();
  const gone: string[] = [];
  const crawled: string[] = [];

  const read = async (urls: string[]): Promise<void> => {
    const wanted = urls.filter((url) => kinds.has(url) && !readThisRun.has(url));
    await inParallel(wanted, config.docsFetchConcurrency, async (url) => {
      const entry = kinds.get(url);
      if (!entry || readThisRun.has(url)) return;
      const outcome = await readPage(config, url, entry.kind, entry.sources, meta.get(url), now);
      ...
      if (outcome.notModified) {
        result.unchangedByEtag += 1;
        // Nothing was downloaded, so nothing is written back except the fact
        // that we looked: `fetched_at` is what the next run's tiers read.
        await store.touchPage(url, now).catch(() => undefined);
        return;
      }
      if (!outcome.page) return;
  ...
  const newlyLinked = withCrawl
    .filter(
      (entry) => !meta.has(entry.url) && !readThisRun.has(entry.url) && !gone.includes(entry.url),
    )
    .map((entry) => entry.url);
  if (newlyLinked.length > 0) {
    const room = Math.max(0, config.posthogMaxPages - result.fetched);
    log.info(
  ...
  log.info(
    `corpus: ${result.pages.length} live pages, read ${result.fetched} (${result.changed} changed), ${result.unchangedByEtag} answered 304, retired ${result.retired}`,
  );
  result.notes.push(
    `corpus: ${result.pages.length} live pages, ${result.fetched} read, ${result.changed} changed, ${result.unchangedByEtag} unchanged by etag, ${result.retired} retired`,
  );
  return result;
}
...
function mergeCorpus(
  stored: PostHogPage[],
  readThisRun: Map<string, PostHogPage>,
  retired: Set<string>,
): PostHogPage[] {
  const merged = new Map<string, PostHogPage>();
 ...
 *
 * Only pages that actually name a competitor are re-read, so this costs one
 * request per compare or blog page that mentions Mixpanel or Amplitude rather
 * than one per marketing page in the corpus.
 */
async function refreshClaims(
  ...
  const candidates = [...readThisRun.values()].filter(
    (page) => page.kind === "marketing" && page.mentions.length > 0,
  );
  let claims = 0;

  await inParallel(candidates, config.docsFetchConcurrency, async (page) => {
    try {
      ...
      const found = extractClaims(page.url, extractPage(html).blocks);
      await store.replaceClaimsForUrl(page.url, found);
      claims += found.length;
    } catch (error) {
      log.warn(
        `could not read claims off ${page.url}`,
 ...
 *
 * Run once, on a corpus with nothing in it, because filling a few thousand
 * pages one request at a time is the slowest run this bot will ever do. After
 * that the file is never read again: it is a vendor's export, which means it
 * is as current as whenever they last generated it, and freshness here is
 * decided by re-downloading a page and hashing what comes back.
 */
