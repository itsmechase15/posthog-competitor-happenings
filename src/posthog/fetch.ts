...
import { extractLinks, extractPage, proseText } from "../util/html.js";
import { fetchConditional, fetchText, type CacheValidators } from "../util/http.js";
import { collapseWhitespace, normalizeUrl, titleFromUrl } from "../util/text.js";

const log = createLogger("posthog-fetch");

/** One PostHog page as it came off the network. */
export interface FetchedBody {
   ...
   */
  links: string[];
  /** What the server said about this copy, for the next run's conditional GET. */
  etag: string | null;
  lastModified: string | null;
}

/** The server says the copy we hold is still current, so there is nothing to read. */
export const NOT_MODIFIED = Symbol("not-modified");
export type FetchOutcome = FetchedBody | typeof NOT_MODIFIED;

/**
 * posthog.com serves every docs page as markdown when `.md` is appended, and
...
export function markdownUrlFor(url: string): string | null {
  try {
    ...
    if (parsed.hostname !== "posthog.com" && parsed.hostname !== "www.posthog.com") return null;
    if (!parsed.pathname.startsWith("/docs/")) return null;
 ...
 * sample or an SDK snippet, and quoting one back at a model invites it to
 * reason about JavaScript instead of about what the product does.
 *
 * Links are read off the source before any of that, because the flattened text
...
export function parseDocMarkdown(markdown: string, baseUrl = ""): ParsedMarkdown {
  ...
  if (!title) title = (/^\s{0,3}#\s+(.+?)\s*$/m.exec(body)?.[1] ?? "").trim();
...
/**
 * Read one PostHog page, unless the server says the copy we hold is current.
 *
 * Markdown when the docs host offers it, HTML otherwise, which is how
 * /pricing, the compare pages, and the changelog are read. The validators make
 * the difference between a corpus that costs a few thousand downloads a day
 * and one that costs a few thousand round trips: posthog.com answers
 * `If-None-Match` with a 304 and no body.
 */
export async function fetchPostHogPage(
  config: Config,
  url: string,
  ...
  if (markdownUrl) {
    try {
      const response = await fetchConditional(markdownUrl, {
      ...
      if (response.notModified) return NOT_MODIFIED;

      const parsed = parseDocMarkdown(response.text, url);
  ...
  // The HTML path takes no validators: they belong to whichever URL issued
  // them, and a 304 from the markdown copy says nothing about this one.
