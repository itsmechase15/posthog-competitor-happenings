 ...
 *
 * The list is the source of truth for three things that used to drift apart:
 * `.env.example`, the secrets table in the README, and the preflight the
 * workflows run before the pipeline. Add a variable to `src/config.ts` and it
 * belongs here too.
 */
...
export type Need =
  ...
  | "required"
  /** One competitor source goes dark. The run still posts everything else. */
  | "source"
...
export const REQUIREMENTS: Requirement[] = [
  {
    ...
    howToGet:
      "Supabase → Project Settings → Database → Connection string → Session pooler. It has to be the pooler URI, not the direct host",
  },
  {
    name: "CURSOR_API_KEY",
    need: "required",
  ...
  },
  {
    ...
    need: "required",
    home: "actions-secret",
    purpose: "Posts the alert with chat.postMessage",
    howToGet:
      "api.slack.com/apps → your app → OAuth & Permissions → bot scope chat:write → install → copy the xoxb- token, then /invite the app to the channel",
    without: "nothing is delivered unless SLACK_WEBHOOK_URL is set instead",
  },
  {
    name: "X_BEARER_TOKEN",
    need: "source",
    home: "actions-secret",
    purpose: "Reads the last few posts from @mixpanel and @Amplitude_HQ",
  ...
  },
  {
    ...
    need: "source",
    home: "actions-secret",
    purpose: "Reads the inbox the competitors' newsletters are delivered to",
    howToGet: "agentmail.to → dashboard → API keys",
  ...
  },
  {
    ...
    need: "optional",
    home: "actions-secret",
    purpose: "Fallback delivery for a workspace where creating an app is not worth it",
    howToGet: "api.slack.com/apps → your app → Incoming Webhooks → Add New Webhook to Workspace",
    without: "nothing, as long as there is a bot token. The bot token wins when both are set",
  },
  {
    name: "GITHUB_TOKEN",
    need: "optional",
...
/**
 * Read the environment against the list above. Pure, so the workflows, the CLI,
 * and the tests all judge a missing secret the same way.
 */
export function checkEnv(env: Env): EnvReport {
  ...
  for (const requirement of REQUIREMENTS) {
    ...
    // The bot token is only missing if there is no delivery path at all: a
    // workspace on the webhook fallback is configured, just not the preferred way.
    if (requirement.name === "SLACK_BOT_TOKEN" && has(env, "SLACK_WEBHOOK_URL")) {
      warnings.push(
        "SLACK_BOT_TOKEN is unset, so delivery falls back to SLACK_WEBHOOK_URL. The webhook cannot tell you why Slack refused a message",
      );
      continue;
    }

    // A dry run prints the payloads and writes nothing, so it needs no database.
