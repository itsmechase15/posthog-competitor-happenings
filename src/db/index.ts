 ...
 * Slack, and refusing to post it because a dedupe row could not be written
...
class UnreachableDatabaseFallback implements Store {
  ...
  markSlackPosted(...args: Parameters<Store["markSlackPosted"]>) {
    return this.attempt((store) => store.markSlackPosted(...args));
  }
  ...
  touchPage(...args: Parameters<Store["touchPage"]>) {
    return this.attempt((store) => store.touchPage(...args));
  }
