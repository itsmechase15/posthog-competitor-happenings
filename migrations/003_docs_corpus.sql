-- Make `pages` the source of truth for what PostHog documents.
--
-- Until now `pages` held whatever the sitemap crawl reached inside a 60-page
-- budget, and freshness was one timestamp: a page was re-read when it got old,
-- and there was no way to tell a page that had changed from one that had
-- merely been looked at again. A recommendation is only as good as the corpus
-- behind it, so the corpus now carries its own bookkeeping.
--
--   psql "$DATABASE_URL" -f migrations/003_docs_corpus.sql
--
-- Safe to re-run. Existing rows are backfilled as `docs` pages with no content
-- hash and no cache validators, which makes the next run re-read them once and
-- record what they actually say.
...
-- Which discovery sources offered this URL on the last run: the sitemap,
-- llms.txt, a link on a page we already had, the changelog index, or the
-- catalog, which pins the overview pages the bot routes to.
alter table pages add column if not exists discovered_from text[] not null default '{}';

-- What posthog.com said about the copy we hold. Sending these back as
-- `If-None-Match` and `If-Modified-Since` turns most of a few thousand daily
-- re-reads into a 304 with no body, which is the only reason a corpus this
-- size is affordable to keep current.
